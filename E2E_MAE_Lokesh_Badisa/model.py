...
class ViTMAE(nn.Module):
    ...
    def masking(self, x, mask_ratio):
        if self.center_mask:
            return self.centred_masking(x, mask_ratio)
        else:
    ...
    def centred_masking(self,x,ratio):

        N, L, D = x.shape  # batch, length, dim
        sig = s = int(np.sqrt(L))
        z = get_gauss(s,sig).reshape(-1)
        z = z / z.sum()
        ids_shuffle = np.random.choice(s**2, s**2, replace=False,p=z)
        ids_restore = np.argsort(ids_shuffle)
        ids_shuffle = torch.tensor(ids_shuffle).to(x.device).unsqueeze(0)
        ids_restore = torch.tensor(ids_restore).to(x.device).unsqueeze(0)
        
        len_keep = int(s**2 * (1 - ratio))
        ids_keep = ids_shuffle[:len_keep]

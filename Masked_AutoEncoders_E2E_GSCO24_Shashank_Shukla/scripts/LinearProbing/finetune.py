...
class Trainer:
    def __init__(
        ...
        self.ckp_PATH = f"/global/homes/s/ssshukla/Shashank/scripts/Results/Finetune/Weights/{self.args.model_name}/checkpoint.pt"
        self.logs_PATH = f"/global/homes/s/ssshukla/Shashank/scripts/Results/Finetune/Logs/{self.args.model_name}/"
        self.weights_path = os.path.join(args.MAE_path,f"{self.args.model_name}/checkpoint.pt")
        #--------------------------------------------------------------#
          
        #--------------------------------------------------------------#
        #Load Checkpoints and wrapup with linear linear layer on top classification purposes
        self._load_autoencoder_weight(self.weights_path)
        self.linear_model = Get_linear_model(self.model, 
    ...
    def _run_epoch(self, epoch):
        ...
        for step, batch in progress_bar:
            ...
            if step % 200 == 0:
                with open(f'finetune_logs.txt', 'a') as f:
